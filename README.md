...
以下のパラメータを設定することで任意のシナリオで都市気温のシミュレーションに活用できます。
...
数式や各種理論は、以下の文献を参考にしています。

G.S.Campbellほか：生物環境の物理学基礎，森北出版(2003）  
...
3. シミュレーション条件の設定
  ...
  calcUrbanTemperature.py 内のパラメータを設定します

4. `calcUrbanTemperature.py` を実行
   ```bash
   python calcUrbanTemperature.py
   ```
5. 結果（CSV・グラフ画像）が同じフォルダに出力されます
...
一つ目の式は、24時間の気温変化を0~1の範囲で表現するフーリエ級数です。この式をもとに、二つ目の式で時刻tにおける大気温度を計算します。
...
**解説:**
大気の安定度（乱流の発生しやすさ）を示す無次元量と、乱流輸送の補正係数を計算します。
単位は $\zeta$ が無次元、 $\psi_M$, $\psi_H$ も無次元です。
